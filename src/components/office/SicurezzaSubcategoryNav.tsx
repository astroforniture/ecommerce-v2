...
export function SicurezzaSubcategoryNav({
  products,
  selectedSubcategory,
  onSelect,
  className = '',
  ...
  const counts = Object.fromEntries(
    SICUREZZA_SUBCATEGORIES.map((label) => [
      label,
      products.filter((p) => matchesSicurezzaSubcategoryFilter(p, label)).length,
    ]),
  ) as Record<SicurezzaSubcategory, number>

  return (
    <nav
      className={['flex flex-wrap gap-2', className].filter(Boolean).join(' ')}
      aria-label="Sotto-categorie Sicurezza"
    >
      <SubcategoryPill
